...
trained_models_path = "/home/horacio/git/competition/trained-models/PARL/saved_files"
...
class RLAgent(BaseAgent):
    def __init__(self, action_space):
        ...
        with open(
            os.path.join(trained_models_path, "three_sub_action_to_sub_ids.pickle"),
            "rb",
        ) as f:
    ...
    def save(self, path):
        torch.save(
            self.combined_model_1.state_dict(),
    ...
    def _first_stage_act(self):
        ...
        (
            ...
            done_simulate,
            info_simulate,
        ) = self.observation.simulate(self.do_nothing_action)
    ...
    def _predict_combined_action_rho(self, observation):
        ...
        predicted_rho1 = self.combined_model_1(processed_obs).detach().cpu().numpy()
        predicted_rho2 = self.combined_model_2(processed_obs).detach().cpu().numpy()
        predicted_rho = (predicted_rho1 + predicted_rho2) / 2.0
        return predicted_rho

    def _predict_unitary_action_rho(self, observation):
        processed_obs = self.unitary_feature_processor.process(self.observation)
        processed_obs = torch.as_tensor(processed_obs).view(1, -1)
        predicted_rho = self.unitary_model(processed_obs).detach().cpu().numpy()[0]
        return predicted_rho
    ...
    def _three_sub_action(self):
        ...
        for best_idx in sorted_idx:
            ...
            if not np.all(self.observation.time_before_cooldown_sub[sub_ids] == 0):
                sub_ids = []
                continue
        ...
        for sub_id in sub_ids:
            ...
            self.serial_actions.append((self.action_space.from_vect(act), sub_id))
        ...
        if len(self.serial_actions) > 0:
            ...
            for i, (act, sub_id) in enumerate(self.serial_actions):
                ...
                if np.any(sub_topo == -1):  # line disconnected
                    ...
                    action_topo = legal_action_vec[start:end].astype("int")  # reference
                    action_topo[np.where(sub_topo == -1)[0]] = 0  # done't change bus=-1
